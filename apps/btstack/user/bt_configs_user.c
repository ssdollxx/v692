...
u8 user_addr[6] = {0x77, 0x12, 0x51, 0x19, 0x30, 0x48};
   ...
   // log_printf("btif_len:0x%x\n", app_use_flash_cfg.btif_len);
    ...
    if (cfg_status) {
         ...
         */
            ...
            puts("BD_addr Fixed:");
            memcpy(user_addr, bt_cfg.addr, 6);
            put_buf(adr, 6);
...
static char host_name[LOCAL_NAME_LEN] = "TE";

//char tyty[9] ;

 // vm_read(88, (void*)&tyty[0],4);
...
// konwertuje polskie znaki z UTF-8 i Unicode do Windows1250
#define UTOWIN1250_BUFFER_LENGTH 500
static char uToWin1250_buffer[UTOWIN1250_BUFFER_LENGTH];


char * uToWin1250(const char *str) {
  uint16_t idx = 0;
  uint8_t last = 0;
  while (((*str) != 0) && (idx < UTOWIN1250_BUFFER_LENGTH - 1)) {
    if ((((uint8_t)*str) < 128) && (((uint8_t)*str) != 1)) {
      last = 0;
      uToWin1250_buffer[idx++] = *str;
    }
    else if (((((uint8_t)*str) & 0xC0) == 0xC0) || (0x01 == ((uint8_t)*str))) {
      last = ((uint8_t) * str);
    }
    else if (((last & 0xC0) == 0xC0) || (0x01 == last)) {
      uint16_t word = (last << 8) | ((uint8_t) * str);
      uint8_t character;
      switch (word) {
        case 0x0104:
        case 0xC484: character = 0xA5; break; // Ą
        case 0x0106:
        case 0xC486: character = 0xC6; break; // Ć
        case 0x0118:
        case 0xC498: character = 0xCA; break; // Ę
        case 0x0141:
        case 0xC581: character = 0xA3; break; // Ł
        case 0x0143:
        case 0xC583: character = 0xD1; break; // Ń
        //case 0x00D3:
        case 0xC393: character = 0xD3; break; // Ó
        case 0x015A:
        case 0xC59A: character = 0x8C; break; // Ś
        case 0x0179:
        case 0xC5B9: character = 0x8F; break; // Ź
        case 0x017B:
        case 0xC5BB: character = 0xAF; break; // Ż
        case 0x0105:
        case 0xC485: character = 0xB9; break; // ą
        case 0x0107:
        case 0xC487: character = 0xE6; break; // ć
        case 0x0119:
        case 0xC499: character = 0xEA; break; // ę
        case 0x0142:
        case 0xC582: character = 0xB3; break; // ł
        case 0x0144:
        case 0xC584: character = 0xF1; break; // ń
        //case 0x00F3:
        case 0xC3B3: character = 0xF3; break; // ó
        case 0x015B:
        case 0xC59B: character = 0x9C; break; // ś
        case 0x017A:
        case 0xC5BA: character = 0x9F; break; // ź
        case 0x017C:
        case 0xC5BC: character = 0xBF; break; // ż
        default:
          character = 0;
      }
      if (character != 0) {
        uToWin1250_buffer[idx++] = character;
      }
      last = 0;
    }
    else {
  ...
  uToWin1250_buffer[idx++] = 0;
  return uToWin1250_buffer;
}
...
static void user_get_bt_music_info(u8 type, u32 time, u8 *info, u16 len)
{
   ...
   // }
	...
	if (tab[0] > tab[1]) {
     ...
     min_c = tab[0] / 1000 / 60;
     sec_c = tab[0] / 1000 - (min_c * 60);
     min_t = tab[1] / 1000 / 60;
     sec_t = tab[1] / 1000 - (min_t * 60);



        printf("#TIME: %02d:%02d / %02d:%02d \n", min_c,sec_c,min_t,sec_t );


    }

   // bt_get_battery_value();
}
